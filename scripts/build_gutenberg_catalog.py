...
TARGET_COUNT = int(os.environ.get("GUTENBERG_LIMIT", "1000"))
...
def data_url_svg(title, author, index):
    colors = [
        ("#176b87", "#f3fbfd"),
        ("#7c2d12", "#fff7ed"),
        ("#365314", "#f7fee7"),
    ...
    short_title = clean_text(title)[:48]
    short_author = clean_text(author)[:34]
  ...
  <text x="38" y="78" fill="{fg}" font-family="Arial, sans-serif" font-size="18" font-weight="700">ENGLISH BOOK</text>
  <foreignObject x="38" y="120" width="284" height="170">
    <div xmlns="http://www.w3.org/1999/xhtml" style="font-family:Arial,sans-serif;color:{fg};font-size:32px;font-weight:700;line-height:1.12;word-wrap:break-word;">{escape_xml(short_title)}</div>
  </foreignObject>
  <foreignObject x="38" y="332" width="284" height="64">
...
def preview_for(subjects):
    if not subjects:
        return "Sách tiếng Anh public domain. Thư viện lưu bản in để phục vụ tra cứu, mượn và trả."
    return "Đọc trước: " + "; ".join(subjects[:4])


def parse_book(member_name, member_file, index):
    try:
    ...
    return {
        ...
        "coverImage": data_url_svg(title, author, index),
        "fileUrl": "",
        "fileName": "",
        "extra": {
            "loaiSachKhac": subtype,
            "nguon": "Project Gutenberg public domain metadata",
            "gutenbergId": source_id,
            "docTruoc": preview_for(subjects),
        },
    }
...
def main():
    ...
    with tarfile.open(feed_path, "r:bz2") as archive:
        ...
        for member in members:
            ...
            item = parse_book(member.name, file_obj, len(catalog) + 1)
            if item:
                catalog.append(item)
                if len(catalog) % 100 == 0:
                    print(f"Collected {len(catalog)}/{TARGET_COUNT}", flush=True)
                if len(catalog) >= TARGET_COUNT:
                    break

    if not catalog:
        raise RuntimeError("No public-domain English books collected.")
